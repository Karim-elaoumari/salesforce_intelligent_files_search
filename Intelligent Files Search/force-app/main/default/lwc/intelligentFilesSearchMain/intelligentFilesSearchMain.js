...
.then(result => {
    ...
    this.files = result.map(file => ({
            ...file,
            Url: `${window.location.origin}/lightning/r/ContentDocument/${file.ContentDocumentId}/view`,
...
.catch(error => {
    ...
    handleRowAction(event) {
        ...
        switch (actionName) {
            case 'option_1':
                const filePreviewUrl = window.location.origin+'/lightning/r/ContentDocument/'+ row.ContentDocumentId +'/view';
